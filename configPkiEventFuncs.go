...
func HandlePkiEvent(c *ShosetConn, message msg.Message) error {
	...
	if c.ch.GetPkiRequestAddress() == "127.0.0.1:8003" {
		for k := range c.ch.ConnsSingle {
			fmt.Printf("key[%s] value[%s]\n", k, c.ch.ConnsSingle[k])
	...
	if c.ch.GetIsPki() && evt.GetCommand() == "pkievt" {
	...
	} else if _, ok := c.ch.ConnsSingle[evt.GetRequestAddress()]; ok && evt.Command == "" {
		...
		fmt.Println("I'm ", c.ch.GetBindAddress(), " and I have received a msg for", c.GetRemoteAddress())
