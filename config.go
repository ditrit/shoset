...
func (cfg *Config) InitFolders(name string) (string, error) {
	...
	if err := maybeMkdir(cfg.baseDir + name + PATH_CONFIG_DIR); err != nil {
		return VOID, err
	}
	if err := maybeMkdir(cfg.baseDir + name + PATH_CERT_DIR); err != nil {
		return VOID, err
	}
...
func (cfg *Config) WriteConfig(fileName string) error {
	...
	return cfg.viper.WriteConfigAs(cfg.baseDir + fileName + PATH_CONFIG_DIR + "config.yaml")
}
